...
export const RecurringPlans = () => {
  return (
    <section id="plans" className="relative py-20 md:py-24 px-4 bg-soft-zinc/30 dark:bg-white/[0.02] transition-colors duration-500">
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <motion.div
                ...
                className={`relative group ${plan.popular ? "md:-translate-y-3" : ""}`}
              >
                {plan.popular && (
                ...
                <div
                  className={`double-bezel rounded-4xl p-1.5 transition-all duration-700 ${
                    plan.popular
                ...
                >
                  <div className="double-bezel-inner relative bg-pristine dark:bg-obsidian rounded-[1.625rem] p-8 md:p-10 h-full flex flex-col gap-6 transition-colors duration-500">
                    <div className="w-14 h-14 rounded-2xl bg-obsidian/5 dark:bg-white/10 flex items-center justify-center transition-colors duration-500">
                      <Icon size={26} strokeWidth={1.25} className="text-obsidian dark:text-white" />
                    </div>
                    <div className="space-y-3 flex-1">
                      <h3 className="text-2xl md:text-3xl font-display font-medium tracking-tight text-obsidian dark:text-white transition-colors duration-500">
                        {plan.title}
                      </h3>
        ...
        >
          <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {inclusions.map((item) => (
              <li
                key={item}
                className="inline-flex items-center gap-2 text-sm font-medium text-obsidian/60 dark:text-white/60 transition-colors duration-500"
              >
